...
const DecoratorBlob = tw(
  SvgDotPattern
)`w-20 h-20 absolute right-0 bottom-0 transform translate-x-1/2 translate-y-1/2 fill-current text-gray-500`;
...
const Description = tw.p`mt-8 text-center md:text-left text-sm md:text-base lg:text-lg font-medium leading-relaxed text-gray-100`;
...
const FeatureIconContainer = styled.div`
  ${tw`mx-auto inline-block border border-blue-500 text-gray-500 text-center rounded p-2 flex-shrink-0`}
  ${props => [
    props.iconRoundedFull && tw`rounded-full`,
    props.iconFilled && tw`border-0 bg-red-500 text-gray-100`
  ]}
  svg {
    ${tw`w-5 h-5`}
  }
